...
func SetDefaultValidateTranslator(translator *ValidateTranslator) {
	_defaultValidateTranslator = translator
}

func GetDefaultValidateTranslator() *ValidateTranslator {
	return _defaultValidateTranslator
}
...
func NewValidateTranslator(validate *validator.Validate) (*ValidateTranslator, error) {
...
func (tl *ValidateTranslator) ValidateErr(locale string, err error) string {
...
func (tl *ValidateTranslator) TranslateFirst(locale string, err error) string {
	...
	if ok := errors.As(err, &e2); ok {
		return tl.ValidateErr(locale, err)
	}

	if ok := errors.As(err, &e1); ok {
		if len(e1) > 0 {
			return tl.ValidateErr(locale, e1[0])
		}
	}
...
func TranslateValidateErr(locale string, err error) string {
	return _defaultValidateTranslator.ValidateErr(locale, err)
}
